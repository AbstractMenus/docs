...
For Path 2 addons (new in 2.0), you don't call `get()` at all — the API is passed as a parameter to your `MenuExtension` lifecycle methods. See [addon delivery](/docs/en/developers/addons/).
...
The static `Handlers` facade is gone. Each section is now a [`ProviderSection<T>`](/docs/en/developers/handlers/) on `api.providers()`:
...
If you're on GitHub Packages instead of JitPack, the native coord is `ru.abstractmenus:api:2.0.0-alpha.2`. See [Getting started](/docs/en/developers/general/) for the full setup.
...
<CardGrid>
  <LinkCard title="Addon delivery" href="/docs/en/developers/addons/" description="2.0 ships two paths: plugin-as-addon, AM-loaded jar. Pick one." />
  <LinkCard title="Custom types" href="/docs/en/developers/own-types/" description="Updated examples for actions, rules, activators, properties, catalogs." />
  <LinkCard title="Provider handlers" href="/docs/en/developers/handlers/" description="ProviderRegistry, multi-provider coexistence, priority-based resolve." />
</CardGrid>
