...
To get values which player entered, you need to use context placeholders, as described [above](/docs/advanced/input/). Just see which placeholders the activator provides and add the `activator_` prefix in begin. Example:
...
Let's modify our example, which we made [before](/docs/advanced/input/). We will replace `shiftClickEntity` activator to `command` and make command take player name:
