...
Access to context data via placeholders is possible thanks to Value Extractors. More about value extractors is in the [Value extractors](/docs/general/placeholders/#value-extractors) section. Here we just describe **how to use a specific extractor as a placeholder**.
...
Each activator placeholder starts with the `activator_` prefix. The `<extractor placeholder>` is a placeholder defined by the specific extractor (see [Value extractors](/docs/general/placeholders/#value-extractors)).

### How to use a context placeholder

<Steps>

1. Open the [activators reference](/docs/general/activators/).
...
Here we used the `entity_custom_name`, `entity_loc_x`, `entity_loc_y` and `entity_loc_z` extractor placeholders, defined in the [Entity extractor](/docs/general/placeholders/#entity-extractor) reference.
...
We used regular placeholders, because Entity Extractor accepts them when the entity is a player. This and other information about each extractor is in the [Value extractors](/docs/general/placeholders/#value-extractors) section.
...
More complex examples of context placeholders usage are in the [Examples](/docs/general/examples/) section.
...
This section covers the more complex form of the `command` activator with typed arguments. Other information about this activator is on the [Activators](/docs/general/activators/) page.

### Command format

To build a command with arguments, declare the `command` activator as an [object](/docs/start/hocon/#object). The object has several required and optional parameters:
...
The `command` activator uses the [Command extractor](/docs/general/placeholders/#command-extractor) to surface entered values to placeholders. An example of using this extractor is in the section below.
...
The only required parameter is `into`. This is the name of a [variable](/docs/general/variables/). When the player enters text in chat, the plugin saves it into a variable with the specified name. After that, you can read the variable from anywhere through a [variable placeholder](/docs/general/variables/#access-to-variables).
