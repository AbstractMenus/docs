...
description: "Generated menu can be created in few steps:"
---
...
1.  Specify the catalog of objects. It can contain additional parameters for filtering objects, etc.
2.  Create a matrix of cells, into which items from the catalog will be placed according to the template. The matrix should have as many rows as the menu itself.
3.  Specify item templates. It is these items that the necessary placeholders from the catalog are indicated.
4.  The generated menu might have several pages. Special actions can be used to navigate between them.
...
Returns all online players on the server. Since this catalog uses `extractor-entity` and all objects in provided collection are Players, you can use any regular placehodler with `ctg_` prefix when you use this catalog type. Example:
